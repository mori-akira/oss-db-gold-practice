...
const addPracticeCompleteBox = () => {
  $("h2, h3").each((_, e) => {
    ...
    const box = $(
      `<input type='checkbox' id='toggle-complete-${id}' class='toggle-box' data-target='${id}'>`
    );
    ...
    div.append(
      $(
        `<label for='toggle-complete-${id}' class='toggle-box-label'>学習済み</label>`
      )
    );
    $(e).append(div);
  });
...
const createContentTree = () => {
  ...
  $("h2, h3").each((_, e) => {
    ...
    if ("H2" === $(e).prop("tagName")) {
      ...
      details = $(
        `<details data-for-header='${id}'><summary><a href="#${id}">${$(e)
          .children("span")
          .text()}</a></summary></details>`
      );
      ul = $("<ul></ul>");
      ...
      if (ul) {
        ul.append(
          $(
            `<li data-for-header='${id}'><a href="#${id}">${$(e)
              .children("span")
              .text()}</a></li>`
          )
        );
...
const updatePracticeProgress = () => {
  const total = $.makeArray($("h2, h3")).reduce(
    (pv, cv) => pv + ($(cv).attr("id").startsWith("header-") ? 1 : 0),
    0
  );
  const done = Object.values(practiceProgressData).reduce(
    (pv, cv) => pv + (cv ? 1 : 0),
    0
  );
...
const initPracticeProgress = async () => {
  ...
  if (option) {
    Object.keys(practiceProgressData).forEach(
      (e) => (practiceProgressData[e] = false)
    );
    await PracticeProgressDao.add(practiceProgressData);
    $.makeArray($("input"))
      .filter(
        (e) =>
          $(e).attr("id").startsWith("toggle-complete-") && $(e).prop("checked")
      )
...
$(async () => {
  ...
  practiceProgressData =
    (await PracticeProgressDao.readLatest())?.progress ?? {};
